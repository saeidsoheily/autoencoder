...
"""
Autoencoder: Image denoising autoencoder based on a 2d-convolutional neural network [using TensorFlow 2.x] (MNIST) 
"""
import numpy as np
...
def load_data():
    '''
    Load MNIST dataset
    :return: X_train, X_train_n, y_train, X_test, X_test_n, y_test, img_size:
    '''
    ...
    # Add noise to data
    X_train_n = np.clip(X_train + 0.3 * np.random.normal(loc=0.0, scale=1.0, size=X_train.shape), 0., 1.)
    X_test_n = np.clip(X_test + 0.3 * np.random.normal(loc=0.0, scale=1.0, size=X_test.shape), 0., 1.)
...
if __name__ == '__main__':
    # Load mnist digits dataset (+ create noisy data)
    X_train, X_train_n, y_train, X_test, X_test_n, y_test, img_size = load_data()
    ...
    autoencoder_model.fit(X_train_n, X_train, # pair of noisy data and original data
                          epochs=iter_number,
    ...
    X_pred = autoencoder_model.predict(X_test_n)
    ...
    # Plot sample noisy data
    plot_sample_data(X_test_n, y_test, img_size, axes[1],
                     tag='NOISY ',
                     color_map='viridis',
                     onehotencoder_label=False)  # plot original image

    # Plot sample denoised data using autoencoder model
    ...
    plt.savefig('tensorflow_keras_denoising_cnn2d.png', bbox_inches='tight')
    plt.show()